...
from suspiciondetector import SuspicionDetector
...
@torch.no_grad()
def run(args):
    suspicion_detector = SuspicionDetector()
    ...
    for frame_idx, r in enumerate(results):
        ...
        if r.boxes.data.shape[1] == 7:
            ...
            for idx, cls in enumerate(r.boxes.cls):
                if int(cls) == 0:
                    detected_current.append(int(r.boxes.id[idx]))

            suspicion_detector.add(detected_current, r.orig_img)
